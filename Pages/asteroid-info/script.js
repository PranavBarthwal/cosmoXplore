...
function neows (inputUrl, sdi) {
  fetch(inputUrl)
    ...
    .then(data => {
      ...
      let estDiameterMinimum =
        data.near_earth_objects[sdi][0].estimated_diameter.kilometers
          .estimated_diameter_min
      let estDiameterMaximum =
        data.near_earth_objects[sdi][0].estimated_diameter.kilometers
          .estimated_diameter_max
      let potentialHazard =
        data.near_earth_objects[sdi][0].is_potentially_hazardous_asteroid
      let closeApproachDate =
        data.near_earth_objects[sdi][0].close_approach_data[0]
          .close_approach_date
      let relativeVelocitySec =
        data.near_earth_objects[sdi][0].close_approach_data[0].relative_velocity
          .kilometers_per_second
      let relativeVelocityHr =
        data.near_earth_objects[sdi][0].close_approach_data[0].relative_velocity
          .kilometers_per_hour
      let orbitingBody =
        data.near_earth_objects[sdi][0].close_approach_data[0].orbiting_body
      ...
      document.querySelector('#estDiaMin').innerHTML =
        estDiameterMinimum.toFixed(3)
      document.querySelector('#estDiaMax').innerHTML =
        estDiameterMaximum.toFixed(3)
...
searchBtn.addEventListener('click', () => {
  ...
  } else [alert('Date input cannot be empty')]
})
