...
document.querySelector("#getPic").addEventListener('click', getImage)
let key = "eISHzmGbQk7EMWv9NTlVtvzZVmeLKPUoL3uKMSPJ";
let date = new Date();
...
//function to display data in apod section on a particular date
function getImage() {
...
function displayData(media, info, title, date, copyright, mediatype) {
  if (mediatype === "video") {
    ...
    document.getElementById(
      "apod_video"
    ).innerHTML = `<iframe width="100%" height="100%" src="${media}" frameborder="0" allowfullscreen></iframe>`;
  } else {
