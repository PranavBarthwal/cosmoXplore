...
function miniGallery (data) {
  let galleryContainer = document.querySelector('div#galleryContainer')

  // extract only 6 pictures'URL
  for (i = 1; i <= 6; i++) {
    let url = data.collection.items[i].links[0].href

    let img = document.createElement('img')
    img.classList.add('mini-gallery-media')
    img.src = url
  ...
  // button will redirect to the main gallery page
  galleryContainer.appendChild(showMoreButton)
}

// onclicking the show-more button, user will be redirected to the main gallery page
